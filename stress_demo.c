...
void producer_work(int mq_id, int p_id) {
    ...
    for (int i = 0; i < LOOP_COUNT; i++) {
        p.id = p_id;
        p.seq = i;
        // Ghi dấu nhận diện độc quyền để kiểm tra xem có bị rò rỉ sang tiến trình khác không
        printf("-> [P_%d] Dang gui tin nhan luot %d (Type %d)...\n", p_id, i, type);
        
        // msgsnd() sẽ tự động sleep nếu hàng đợi chạm ngưỡng MAX_MSG (10)
        if (msgsnd(mq_id, type, (char*)&p) < 0) {
            printf("[ERR] P_%d gui that bai o luot %d\n", p_id, i);
            exit(1);
        }
    }
    printf("[OK] Producer_%d da hoan thanh nhiem vu ban pha.\n", p_id);
    exit(0);
}
...
void consumer_work(int mq_id, int c_id) {
    ...
    for (int i = 0; i < LOOP_COUNT; i++) {
        ...
        if (msgrcv(mq_id, wanted_type, (char*)&p) < 0) {
            printf("[ERR] C_%d nhan that bai o luot %d\n", c_id, i);
            exit(1);
        }
        ...
        if ((p.id % 2 == 0 && wanted_type != 1) || (p.id % 2 != 0 && wanted_type != 2)) {
            printf("[CRITICAL] RO RI DU LIEU SAI LECH! Consumer %d nhan nham data cua Producer %d\n", c_id, p.id);
            exit(1);
        } else {
            printf("<- [C_%d] Nhap va xac thuc OK: Data tu P_%d, chuoi thu %d (Type %d)\n", c_id, p.id, p.seq, wanted_type);
        }
    }
    printf("[OK] Consumer_%d da tieu thu va xac thuc sach se.\n", c_id);
    exit(0);
}
