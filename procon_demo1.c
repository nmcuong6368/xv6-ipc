...
int main(int argc, char *argv[]) {
    ...
    // 2. Tạo tiến trình Producer bắn phá dữ liệu vào hàng đợi trước
    if(fork() == 0) {
        producer_process(mq_id);
    }

    // Tiến trình cha đợi đồng bộ: Ép Producer phải chạy xong và giải phóng Console hoàn toàn
    wait(0);

    // 3. Sau khi Producer đã hoàn tất, tiến trình cha mới tạo tiếp luồng Consumer
    if(fork() == 0) {
        consumer_process(mq_id);
    }

    // Tiến trình cha đợi Consumer hoàn thành chu trình xử lý của mình
    wait(0);
